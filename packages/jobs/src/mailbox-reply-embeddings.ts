  ...
  async ({ logger }) => {
    ...
    for (const batch of chunk(usable, EMBED_CHUNK_SIZE)) {
      ...
      try {
        ...
        logger.error(
          `[mailbox-reply-embeddings] embedding call failed: ${name} (chunk of ${batch.length})`,
        )
        throw new Error(
          `mailboxReplyEmbeddingsJob: embedding call failed (${name}) for a chunk of ${batch.length} messages`,
        )
      }
