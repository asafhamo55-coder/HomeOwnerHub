...
Bring a neighbor, bring the kids, and let's make {{association_name}} look its best together. We're grateful for every hour you can spare on a Saturday.$tpl$,
    $tpl$[{"id":"event_date","label":"What Saturday is Cleanup Day?","type":"date","required":true},{"id":"start_time","label":"What time does it start?","type":"time","required":false},{"id":"end_time","label":"What time does it wrap up?","type":"time","required":false},{"id":"meeting_location","label":"Where should volunteers meet?","type":"text","required":false,"help":"e.g. \"the clubhouse parking lot\""},{"id":"rsvp_instructions","label":"How should residents let you know they are coming?","type":"select","options":["No RSVP needed — just show up","Reply to this email to let us know","Sign up at the clubhouse front desk","Sign up using the community app"],"required":false}]$tpl$::jsonb,
...
Questions about where guests can park? Reach out to {{report_contact}} and we are happy to help.$tpl$,
    $tpl$[{"id":"reason","label":"What is driving this reminder?","type":"select","options":["shared guest spaces reaching capacity","guests parking in spaces reserved for residents","vehicles left in guest spaces for multiple days","contractor and vendor trucks taking up guest spaces"],"required":false},{"id":"max_guest_stay","label":"How long may a guest vehicle stay?","type":"select","options":["up to 24 hours","up to 48 hours","up to 72 hours","no overnight guest parking"],"required":false},{"id":"affected_areas","label":"Which areas are reserved or off-limits to guests?","type":"multiselect","options":["spaces marked reserved for residents","fire lanes and no-parking zones","the spaces nearest the clubhouse","the visitor lot at the main entrance","accessible spaces without a permit"],"required":false,"help":"Select every area guests should avoid right now."},{"id":"report_contact","label":"Who should residents contact with questions?","type":"text","required":false,"help":"e.g. \"the front office at 555-0134\" or \"the HOA management company\""}]$tpl$::jsonb,
...
Questions about what's needed or how to submit it? Reach out to {{contact_name}} and we'll help you get sorted.$tpl$,
    $tpl$[{"id":"deadline_date","label":"Paperwork deadline","type":"date","required":true,"help":"The last day paperwork can come in and still guarantee a pass by opening day."},{"id":"required_items","label":"What does each household need to submit?","type":"multiselect","options":["a completed household roster","a signed liability waiver for all household members","a fob or key request form","a current emergency contact card"],"required":false,"help":"Select everything you need back before you can issue a pass."},{"id":"submission_method","label":"How should households submit their paperwork?","type":"select","options":["drop-off at the clubhouse office","email to the property manager","upload through the resident portal","mail to the management office"],"required":false},{"id":"contact_name","label":"Who should residents contact with questions?","type":"text","required":false,"fallback":"the management office","help":"e.g. \"Jamie in the front office\" or \"the property manager\""}]$tpl$::jsonb,
...
Thanks for keeping the curb clear between pickups — most of us already do, and it makes a real difference for the whole street.$tpl$,
    $tpl$[{"id":"collection_day","label":"What day is trash and recycling collected?","type":"select","options":["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],"required":false},{"id":"early_out_time","label":"How early may bins go out to the curb?","type":"time","required":false,"help":"The earliest time on the evening before pickup, e.g. 6:00 PM."},{"id":"late_in_time","label":"By when must bins be back out of sight?","type":"time","required":false,"help":"The deadline on collection day itself, e.g. 8:00 PM."},{"id":"storage_location","label":"Where should bins be stored the rest of the week?","type":"select","options":["in the side yard","in the garage","behind a fence or screen","in a designated enclosure or pad"],"required":false,"help":"Pick whichever matches your community rule."},{"id":"holiday_note","label":"Any upcoming holiday schedule change to mention?","type":"textarea","required":false,"fallback":"There are no holiday schedule changes at this time.","help":"e.g. \"Collection will run one day late the week of Labor Day.\" Leave blank if none."}]$tpl$::jsonb,
...
We'll send an update if the schedule changes. Questions in the meantime? Just reply to this email.$tpl$,
    $tpl$[{"id":"reason","label":"What kind of work is happening?","type":"select","options":["paving","landscaping work","roof work","utility work","road work","an amenity closure"],"required":true},{"id":"affected_areas","label":"Which area is affected?","type":"text","required":false,"help":"Name the exact spot — e.g. \"the visitor lot and the north driveway\" — specifics are what tell someone whether this affects them."},{"id":"start_date","label":"When does the work begin?","type":"date","required":true},{"id":"end_date","label":"When is the work expected to wrap up?","type":"date","required":true,"help":"An estimate is fine — the email phrases this as expected, not guaranteed."},{"id":"parking_instructions","label":"What do residents need to do before work starts?","type":"textarea","required":false,"help":"Be specific: which vehicles need to move, where to park instead, and by what date or time. This is the single most useful line in the email."}]$tpl$::jsonb,
...
Not sure whether your project needs approval? Ask before you order materials. We would much rather answer a quick question than review a finished deck.$tpl$,
    $tpl$[{"id":"project_types","label":"What needs approval here?","type":"multiselect","options":["fences and walls","decks, patios and pergolas","exterior paint colour changes","roof replacement and material changes","sheds and outbuildings","solar panels, satellite dishes and antennas"],"required":false,"help":"Take this from the architectural guidelines rather than memory — an incomplete list here is read as permission for anything not on it."},{"id":"submission_route","label":"How do owners submit?","type":"text","required":false,"help":"The form, the portal, or the email address — plus what has to accompany it (plans, materials, a survey)."},{"id":"review_window","label":"How long does review take?","type":"text","required":false,"help":"Use the period in your documents. Many set a deadline after which an unanswered application is deemed approved."}]$tpl$::jsonb,
...
If keeping up with your yard is difficult right now — illness, travel, cost, a landscaper who stopped showing up — reply to this email. We would far rather help you find a solution than send a violation notice.$tpl$,
    $tpl$[{"id":"season_context","label":"What is the occasion?","type":"select","options":["the spring growing season","the summer months","fall leaf drop","the annual property walk-through"],"required":false},{"id":"upkeep_items","label":"What needs attention?","type":"multiselect","options":["grass height and edging","weeds in beds and along walkways","overgrown shrubs blocking sidewalks","dead or fallen limbs","leaves left in gutters and drains","bare or eroding areas of lawn"],"required":false},{"id":"deadline_date","label":"When do walk-throughs begin?","type":"date","required":true,"help":"Give people a realistic window — at least two weekends, and more if a landscaper is needed."}]$tpl$::jsonb,
...
If something is wrong with the pool itself — cloudy water, a broken gate latch, missing safety equipment — reply immediately. A gate that does not latch is the one problem here that cannot wait.$tpl$,
    $tpl$[{"id":"guest_policy","label":"What is the guest rule?","type":"text","required":false,"help":"e.g. \"up to four guests per household, and a resident must be present with them\""},{"id":"supervision_policy","label":"What is the child supervision rule?","type":"text","required":false,"help":"Use the age from your pool rules, not a guess — this is the rule most likely to be quoted back to you."},{"id":"pool_hours","label":"Pool hours","type":"text","required":false}]$tpl$::jsonb,
...
For anything genuinely dangerous — downed power lines, a gas smell, structural damage, water rising into a home — call 911 first. Do not wait on the association, and do not reply to this email and wait for us to see it.$tpl$,
    $tpl$[{"id":"weather_event","label":"What are you preparing for?","type":"select","options":["severe thunderstorms","a tropical storm or hurricane","a winter storm and freezing temperatures","the coming storm season","high winds"],"required":false},{"id":"items_to_secure","label":"What should residents secure?","type":"multiselect","options":["patio furniture and umbrellas","trash and recycling bins","grills and fire pits","garden decorations and planters","trampolines and play equipment","holiday decorations and flags"],"required":false},{"id":"responsibility_split","label":"Who handles what afterwards?","type":"textarea","required":false,"help":"The most useful thing in this email. State plainly what the association clears versus what an owner is responsible for, and the number to call for each."}]$tpl$::jsonb,
...
A practical note: whatever the association's rules, your mortgage, your insurer and the county may each have their own position on short-term rentals, and those are not the association's to waive. Check all three before you list.$tpl$,
    $tpl$[{"id":"policy_summary","label":"What do the documents actually say?","type":"textarea","required":false,"help":"Quote or closely paraphrase the leasing provision, including any minimum lease term. If the documents are genuinely silent or ambiguous, say so plainly rather than stating a position the board wishes were true."},{"id":"owner_next_step","label":"What should an owner do first?","type":"textarea","required":false,"help":"e.g. \"please contact the board before listing, so we can confirm whether the lease cap has room and register the tenancy.\""}]$tpl$::jsonb,
...
If you are expecting a delivery, a contractor, or a stretch of guests, reply to this email and let us know. A heads-up prevents almost every complaint we receive.$tpl$,
    $tpl$[{"id":"parking_issue","label":"What is the problem?","type":"multiselect","options":["vehicles parked in the roadway overnight","cars blocking driveways or mailboxes","parking too close to corners and fire hydrants","vehicles that have not moved in weeks","trailers, boats or RVs parked in view","commercial vehicles parked overnight"],"required":false},{"id":"affected_areas","label":"Where is it worst?","type":"multiselect","options":["the main entrance","the cul-de-sacs","the clubhouse lot","the visitor spaces","the streets near the playground","throughout the community"],"required":false},{"id":"parking_guidance","label":"What should residents do instead?","type":"textarea","required":false,"help":"State the rule plainly — e.g. \"Park in your driveway or garage first. On-street parking is for guests, and never overnight.\""}]$tpl$::jsonb,
